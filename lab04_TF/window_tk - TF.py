...
class TicTacToeApp(tk.Tk):
    ...
    def update(self):
        ...
        if board.is_winner(self._board, 'X'):
            ...
            computer_marker = 'O'
            if self._current_marker == computer_marker:
                max_eval = -10000
                for r in range(3):
                    for c in range(3):
                        if board.is_empty(self._board,r, c):
                            board.place(self._board, computer_marker, r, c)
                            board.print_board(self._board)
                            if (e := board.eval(self._board,computer_marker)) > max_eval:
                                max_eval = e
                                best_pos = r,c
                            board.remove(self._board, r, c)
                            print(e)
                            print()
                self.clicked(*best_pos)
